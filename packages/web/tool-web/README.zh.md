...
| 工具 | 参数 | 行为 |
|---|---|---|
| `dsh_web_search` | `queries`（必填 string[]） | 用于发现信息。返回可选答案与来源 URL。它会并发执行 1 至 `searchMaxQueries` 个不同搜索，按轮询顺序合并来源，再应用组合后的 `searchMaxResults` 上限。单元素数组执行一次搜索。完全相同的查询只执行一次。任何搜索失败都会中止批次中的其余搜索；批次结算完毕后调用才返回错误。两个上限都不面向模型。 |
...
| 配置键 | 默认值 | 含义 |
|---|---|---|
| `search` | `true` | 注册 `dsh_web_search`。 |
| `fetch` | `true` | 注册 `web_fetch`。 |
| `searchMaxResults` | `8` | 一次 `dsh_web_search` 调用返回的来源数量上限（seam 截断各提供方列表；工具还会限制多查询组合列表）。 |
| `searchMaxQueries` | `4` | 一次 `dsh_web_search` 调用接受的查询数量上限。配置值会出现在提示词指引与 schema 描述中。 |
| `fetchTimeoutMs` | `30000` | `web_fetch` 的协作式工具调用超时预算（ms）。 |
| `searchTimeoutMs` | `30000` | `dsh_web_search` 的协作式工具调用超时预算（ms）。 |
...
模型会看到生成的 [`dsh_web_search` 与 `web_fetch` schema](../../../docs/tool-catalog.zh.md#deepseek-aidsh-tool-web)。结果数量与超时预算属于部署设置，不是模型参数。
...
多查询调用中的任何查询失败时，`dsh_web_search` 会中止其余搜索，等待所有已启动搜索结算，丢弃成功结果，并针对首次失败返回 `Error: <message>`。
