...
\`run_code\` takes two required arguments: \`code\` — the body of an async Python function (top-level \`await\` and \`return\` both work) — and \`description\`, a short UI label that is never a substitute for \`code\`. At run time exactly two of the names declared below are bound: \`tools\` and \`ToolCallError\`. Everything else is a STATIC STUB describing argument and return types — in particular the \`TypedDict\` classes do NOT exist at run time, so build arguments as plain \`dict\`/\`list\` JSON values: \`await tools.name({"field": 1})\`, never \`FooArgs(field=1)\`, which raises \`NameError\`. Inside the program:

- Always pass both \`code\` and \`description\` on every call, including retries and the next call after a successful one. Never call \`run_code\` with only \`description\` — that fails with \`missing required property "code"\`. Valid: \`{"code":"r = await tools.glob({\\"pattern\\": \\"*\\"}); return r","description":"List workspace files"}\`. Invalid: \`{"description":"List workspace files"}\`.
