...
describe('web command-line provider', () => {
  ...
  it('accepts the all-interfaces host and publishes it to the webStartup service', async () => {
  ...
  it('rejects an unsupported host before the consumer activates', async () => {
    const { values, observed } = await bootProvider(['--host', 'example.com'])
    expect(observed.out).toContain('--host must be either 127.0.0.1 or 0.0.0.0')
    expect(values).toBeUndefined()
    expect(observed.readerConfig).toBeUndefined()
    expect(observed.exits).toEqual([1])
  })
