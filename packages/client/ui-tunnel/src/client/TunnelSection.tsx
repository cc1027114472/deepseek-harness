  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus((prev) => ({
          ...prev,
          error: errJson.error ?? `启动失败 (${res.status})`,
        }))
      }
    } catch (err) {
      setStatus((prev) => ({
        ...prev,
        error: (err as Error).message ?? '网络连接错误',
      }))
    } finally {
      setLoading(false)
    }
  ...
  const handleStop = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus((prev) => ({
          ...prev,
          error: errJson.error ?? `停止失败 (${res.status})`,
        }))
      }
    } catch (err) {
      setStatus((prev) => ({
  ...
  const handleInstall = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus((prev) => ({
          ...prev,
          error: errJson.error ?? `安装失败 (${res.status})`,
        }))
      }
    } catch (err) {
      setStatus((prev) => ({
  ...
  const handleResetToken = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus((prev) => ({
          ...prev,
          error: errJson.error ?? `重置密钥失败 (${res.status})`,
        }))
      }
    } catch (err) {
      setStatus((prev) => ({
