...
- **Always mode retries permanent failures** — authentication, invalid-request, protocol, and unrecoverable context errors continue until success, cancellation, or disposal; deployments own provider-specific cost and latency controls. Default normal mode already retries `QUOTA` with its finite budget.
