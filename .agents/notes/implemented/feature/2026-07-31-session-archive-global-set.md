...
Archived sessions are listed under the sidebar **Archived** tab; each row's menu offers **Unarchive** (`workspace.unarchiveSession`), which removes the id from the global set and restores the row in its prior accounting slot. Session deletion remains out of scope. The `workspace.list` response shape change is a pre-release direct edit (no compatibility layer). The workspace-management e2e pins the full chain (archive → row disappears → still hidden after reload, log still present); domain tests pin idempotence, unknown-id rejection, restart recovery, and the pre-field media default upgrade.
