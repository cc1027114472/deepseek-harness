...
传输自身的 `description`、两种 flavor 的 SDK 说明，以及靠前的 `tools:code-only` 规则都以点名 `code` 和 `description` 这两个必填参数开头，并声明 `description` 只是 UI 标签。把该调用描述成「传入一个程序」的散文会让第二个参数只能从参数 schema 中发现，而只发出 `{code}` 的模型会因 `INVALID_ARGS` 被拒，连同已写好的整个程序一起丢失。反向失败——只发出 `{description}`（弱模型把 UI 标签当成整次调用，或在一次成功后的后续调用里漏掉 `code`）——会以 `missing required property "code"` 被拒。靠前规则与 SDK 第一条 bullet 点名该失败，并配对一个合法参数对象与该非法对象，让模型能按格式抄。由于部分 Gemini 系列模型即使看过这些提示也会在重试后回退，注册表还会只在这条精确失败路径上追加一条很窄的纠正性插件上下文，让下一步重新点名这两个必填字段，而不必让每次请求都永久背上更长的提示词段落。
