...
归档后侧边栏「已归档」标签可查看；行菜单提供「取消归档」（`workspace.unarchiveSession`），从全局集合移除 id 并按原会计 slot 恢复显示。Session 删除仍不在本期范围。`workspace.list` 响应形状变化是 pre-release 直改（无兼容层）。e2e（workspace-management）钉住了「归档→行消失→reload 后仍隐藏、日志仍在」的全链路；domain 层测试钉住幂等、未知 id 拒绝、跨重启恢复与旧介质默认升级。
